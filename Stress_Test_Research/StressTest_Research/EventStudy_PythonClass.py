...
# TODO: Create Python Class Library --Inprogress
...
#Create into Class Library


class EventStudyResearch():
    def __init__(self):
        print("Initialize Objects")
        #print("Set Data Working Directories")

    #Variables for FSB is Year Range and Policy Areas
    def fsb_events(self, yearrange = "2000:2019", policyareas = '''"non-bank-financial-intermediation", "vulnerabilities-assessments"'''):
    ...
    def eba_st_events(self, yearrange = "2009:2019", urlbase = '''"https://www.eba.europa.eu/risk-analysis-and-data/eu-wide-stress-testing"'''):
    #EBA Stress Testing
    ...
    def eba_pr_events(self, urlbase = '''"https://www.eba.europa.eu/news-press/news"'''):
        ...
        return(eba_events)
    #IMF FSAP
    def imf_fsap_events(self, urlbase = '''"https://www.imf.org/en/Publications/Search?series=IMF+Staff+Country+Reports&when=After&title=Financial+System+Stability+Assessment&series=Global%20Financial%20Stability%20Report%20&series=Technical%20Notes%20and%20Manuals"'''):
    ...
    def frb_events(self, urlbase = 'https://www.federalreserve.gov', st_urlbase = 'https://www.federalreserve.gov/supervisionreg/',ccaryearrange = "2011:2018", dfastyearrange = "2013:2018" ):
    ...
    def getevents_data(self, eventscolumnlist = ['title','date','country','category','source']
        ...
        events_combined = self.events_normalize_annctype(events_combined)
    ...
    def events_normalize_annctype(self, events = None, source = ["eba","fsb","imf","frb"]):
    ...
    def DeAgg_Events_Union(self, events_cleaned, BankingUnionColumn="BankingUnion", BankUnion_dict={'BankingUnion': ["European Union", "EuroZone", "CEMAC", "ECCU","European Union","Global"],
    ...
    def get_CountryCodes(self, excelurl = ["http://unstats.un.org/unsd/tradekb/Attachment440.aspx?AttachmentType=1"]
    ...
    def normalize_events_CountryCodes_UN(self, events_cleaned,UN_CountryCodes,findstr_dict = { 'source': ["IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF","IMF"] ,
    ...
    def get_CountryIndices(self, events_cleaned, country_indicies_file = "wrdsWorldIndiciesFIC_Indiicies.csv",gvkey_column = "GVKEY",
    ...
    #Get country regions
    def get_country_regions(self, events_cleaned = None, regionurl = 'https://unstats.un.org/unsd/methodology/m49/', table_element = {'id': 'GeoGroupsENG'},
    ...
    def get_regions_idx(self, region_idx_gvkeyx_dict= {   'World': ['from comp_global.g_idx_index a',"where indexid similar to '%%(WORLD|ALL)%%' and indexval like '%%ALL%%'"],
    ...
    def get_sector_idx(self,  sector_idx_dict = { 'comp_global.g_idx_index':"where indexval in ('BANK','FINANCEOTH','FINANCIAL','FINSRV','FINSVCS','GENFIN','INVEST','MFINSVCS','PORTFOLIO','PRIME','SPECFIN','OTRFIN')"
        ...
        if RunWrdsAPI:
            ...
            if events_cleaned_df is not None:
                ...
                tmp2 = set(events_cleaned_df.ISO3.unique())
    ...
    def get_idx_names(self, events_cleaned, Region = True, Country = True, Sector = True, RunWrdsAPI  =True):
        ...
        if Region:
            ...
            region_idx_names_tmp = self.get_regions_idx(RunWrdsAPI = RunWrdsAPI)
        ...
        if Country:
            ...
            country_idx_names_tmp = self.get_CountryIndices(events_cleaned,RunWrdsAPI = RunWrdsAPI)
            idx_names_df = pd.concat([idx_names_df,country_idx_names_tmp])
        if Sector:
            print("Getting Sector-Level Index Names")
            sector_idx_names_tmp = self.get_sector_idx(events_cleaned_df = events_cleaned,RunWrdsAPI = RunWrdsAPI)
    ...
    def get_idx_prices(self, world_idx_names, events_cleaned, RunWrdsAPI=True, region=True, country=True, sector=True,
            ...
            for i in range(gb.__len__()):
                ...
                for y in range(from_tmp_ls.__len__()):
                    ...
                    where_tmp = "where gvkeyx in ('" + gvkey_tmp + "') " \
                                                                   " and datadate between date '" + str(gb['date']['min'][
                                    i]) + "' - INTERVAL '" + interval_before_mindate + "'" \
                                                                                      " and date '" + str(gb['date']['max'][
        ...
        if region:
            ...
            events_cleaned['date'] = pd.to_datetime(events_cleaned['date'], utc = True)
            ...
            where_tmp = "where gvkeyx in ('" + gvkey_tmp + "') " \
                                                           " and datadate between date '" + str(tmp_date_min) + "' - INTERVAL '" + interval_before_mindate + "'" \
                                                                                                                                                        " and date '" + str(tmp_date_max) + "' + INTERVAL '" + interval_after_maxdate + "';"
            query_tmp = " ".join([select_tmp, from_tmp, where_tmp])
            # Run Query
            if RunWrdsAPI:
                print("Querying Wrds")
    ...
    def Event_Obj_Subsetter(self, EST_Events_Raw, cols = ["source","annctype","regioncode_x","global_idx_tic","regional_idx_tic","ISO3","country_idx_tic","sector_idx_tic"] ):
    ...
    def EST_Event_Generator(self, events_cleaned_df_regions,event_index_names_inprices_df):
    ...
    def EST_File_Generator(self, EST_Events_Raw, event_idx_prices, event_index_names_df,
    ...
    #            tmp_evt_subset_obj[param]= tmp_rules.loc[row,param]
        ...
        #TODO: Need to address duplications as API may not handle so many variations.
            #May need to Pass dup steams to API one at a time.
    ...
    # R API Not working , Service is down 02/06/2019 onward.
    # May need to use API to make requests for better perforamcne and larger data.

    def EST_R_API_Wrapper(self, PreProcess_EST_obj,
                          params_dict={
                              'workingdir': '.',
        ...
        for dup_num in range(1, PreProcess_EST_obj["RequestData"]["dup_stream_num"].max()):
            ...
            PreProcess_EST_obj_rq_tmp[(PreProcess_EST_obj_rq_tmp["firmID"].isin(firmID_match_tmp)) & (
                PreProcess_EST_obj_rq_tmp["marketID"].isin(marketID_match_tmp))].to_csv(params_dict["requestFile"],
                                                                                        sep=';',
            ...
            if run_R:
                ...
                est_api_wrapper = ro.r('''
                    ...
                    estParser$parseRequestFile("''' + params_dict["requestFile"] + '''")

                    estParser$parseReport("./results/analysis_report.csv")
                    estParser$parseAR("./results/ar_results.csv")
                    estParser$parseAAR("./results/aar_results.csv")
                    estParser$parseCAR("./results/car_results.csv")

                    #Output Parsed Results File.

                    write.table(estParser$analysisReport,"./results/analysisReport.csv", sep = ",",row.names = FALSE)
                    write.table(estParser$arResults,"./results/arResults.csv", sep = ",",row.names = FALSE)
                    write.table(estParser$aarResults,"./results/aarResults.csv", sep = ",",row.names = FALSE)
                    write.table(estParser$carResults,"./results/carResults.csv", sep = ",",row.names = FALSE)

            ''')

                print("Reading in Results from API to PY DataFrame")
                estAnalysisReport_tmp = pd.read_csv("./results/analysisReport.csv", sep=',')
                estAnalysisReport_tmp['dup_stream_num'] = dup_num
                estARresults_tmp = pd.read_csv("./results/arResults.csv", sep=',')
                estARresults_tmp['dup_stream_num'] = dup_num
                estAARresults_tmp = pd.read_csv("./results/aarResults.csv", sep=',')
                estAARresults_tmp['dup_stream_num'] = dup_num
                estCARresults_tmp = pd.read_csv("./results/carResults.csv", sep=',')
...
PreProcess_EST_Data_Dict   = EST_File_Generator(EST_Events_Raw,event_idx_prices,event_index_names_df,
                            EstSubset = { "subset_columns" : [["annctype","global_idx","regional_idx","regioncode_x"],
                                      ...
                                      "grouping_var": [["source","regioncode_x"],
                                                        ["source","country"],
                                                        ["source","country","sector_idx"]
                                                       ]
                                    },
...
#May need to build own EST function with statistical significance tests.
#PythonFinace has Event Study functions
#Other R packages
#Manually create functions to comeplete tasks.
...
PreProcess_EST_Data_Dict["RequestData"].shape
PreProcess_EST_Data_Dict["RequestData"].dup_stream_num.max()


request_data_df =PreProcess_EST_Data_Dict["RequestData"]

request_data_df["Event ID"].unique().__len__()

#Backup Object
PreProcess_EST_Data_Dict_RequestData_bk = PreProcess_EST_Data_Dict["RequestData"]
#PreProcess_EST_Data_Dict["RequestData"] = PreProcess_EST_Data_Dict_RequestData_bk

#Set to only stream 1

PreProcess_EST_Data_Dict["RequestData"] = PreProcess_EST_Data_Dict_RequestData_bk[PreProcess_EST_Data_Dict_RequestData_bk["dup_stream_num"].isin(list(range(1,10)))]
