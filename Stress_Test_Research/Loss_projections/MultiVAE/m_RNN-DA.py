...
from MultiVAE import utility as util

global logger

util.setup_log()
util.setup_path(s3_prefix ="None", data_dir= "./")
logger = util.logger
...
class da_rnn:
    def __init__(self, file_data, logger, encoder_hidden_size = 64, decoder_hidden_size = 64, T = 10,
                 learning_rate = 0.01, batch_size = 128, parallel = True, debug = False, use_cuda = False):
        self.T = T
        ...
        X_unorm = dat.loc[:, [x for x in dat.columns.tolist() if x != 'NDX']]
        columns = X_unorm.columns
        X_norm = scale.fit_transform(X_unorm)  # drop the label and normalizing
        X_norm = pd.DataFrame(X_norm)  # It is required to have X converted into a data frame to later plotting need
    ...
    def train(self, n_epochs = 10):
        ...
        for i in range(n_epochs):
            ...
            while j < self.train_size:
                ...
                print("EPOCH:",i, "Loss:",loss, "TrainSize:", self.train_size, "J Size", j, "n_iter:", n_iter)

                #print(i * iter_per_epoch + j / self.batch_size)
    ...
    def train_iteration(self, X, y_history, y_target, use_cuda = False):
    ...
    def predict(self, on_train = False, use_cuda = False):
        if on_train:
...
temp = pd.read_csv('{}/nasdaq100_padding.csv'.format(io_dir), nrows = 10000)
temp.to_csv('{}/nasdaq100_padding_test.csv'.format(io_dir))



model = da_rnn(file_data = '{}/nasdaq100_padding_test.csv'.format(io_dir),logger = logger, parallel = True,
              learning_rate = .001)
