...
def hmc_trajectory(current_z, current_v, U, grad_U, epsilon, L=10, modeltype= "vae", cond = None):
  ...
  for i in range(1, L + 1):
    z = z + v.mul(eps)
    if i < L:
      v = v - grad_U(z,modeltype = modeltype, cond = cond).mul(eps)
...
def accept_reject(current_z, current_v,
                  ...
                  U, K=lambda v: torch.sum(v * v, 1),modeltype = "vae", cond = None):
  ...
  """
  current_Hamil = K(current_v) + U(current_z, modeltype = modeltype, cond = cond)
  propose_Hamil = K(v) + U(z,modeltype = modeltype, cond = cond)
